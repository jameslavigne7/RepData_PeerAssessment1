...
dframe1 <- df %>% filter(!is.na(steps)) %>% group_by(date) %>%
  summarize(totalSteps = sum(steps))
dframe1 <- data.frame(dframe1)
  
hist(dframe1$totalSteps,breaks = 30,main = "Histogram", xlab = "Total Steps")
```
...
```{r}
dframe2 <- df %>% filter(!is.na(steps)) %>% group_by(interval) %>%
  summarize(meanSteps = mean(steps))

dframe2 <- data.frame(dframe2)

plot(dframe2$interval,dframe2$meanSteps, xlab = "Time (minutes)", ylab = "Mean Number of Steps", type = "l")
```
...
```{r, fig.width = 20, figure.height = 30}

dframe5 <- dframe

dframe5$date <- weekdays(as.Date(dframe5$date))

week <- as.character(c("Monday","Tuesday","Wednesday",
"Thursday","Friday","Saturday","Sunday"))

cat <- as.character(c(rep("Weekday",5),rep("Weekend",2)))
...
mergeTable2 <- mergeTable %>% filter(cat == "Weekday") %>%
    group_by(interval) %>%
    summarize(totalSteps = sum(steps))

mergeTable3 <- mergeTable %>% filter(cat == "Weekend") %>%
  group_by(interval) %>%
  summarize(totalSteps = sum(steps))

par(mfrow = c(2,1),mai = c(.8,.8,1,.05))

plot(mergeTable2$interval,mergeTable2$totalSteps,type = "l", main = "Weekdays", xlab = "Time (minutes)",ylab = "Steps Taken")
plot(mergeTable3$interval,mergeTable3$totalSteps,type = "l",main = "Weekends", xlab = "Time (minutes)",ylab = "Steps Taken")
```
